...
function flyToLocation(long, lat, name) {
  if (currentPopup) {
    return;
  } // Don't add new popup if already open
  map.flyTo({
    center: [long, lat],
    zoom: 9,
    speed: 0.8,
...
map.on("load", () => {
  ...
  map.on("click", "unclustered-point", (e) => {
    ...
    if (!comments && !cargo && !hull && !paraphernalia) {
      comments = "No Details";
    }
    ...
    let pattern = new RegExp(
      "\\b(?!" + "Bronze Age" + ")(" + wordsToBold.join("|") + ")\\b",
      "gi"
    );
    document.getElementById("content-box").innerHTML = `<div class="name-text">
    ...
    document.getElementById("comment-box").innerHTML = `${comments.replace(
      pattern,
      "<strong>$&</strong>"
    )} 
                                                        ${cargo.replace(
